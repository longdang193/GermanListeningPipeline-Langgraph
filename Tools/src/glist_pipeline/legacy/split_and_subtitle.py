#!/usr/bin/env python3
"""
split_and_subtitle.py — Split audio and generate SRT subtitles from Listening_2 blocks.

Usage:
    python split_and_subtitle.py <path-to-generated-file.md>
...
def parse_blocks_with_headings(content: str) -> list[dict]:
    ...
    for b_pos, b_text in blocks_raw:
        # Find the latest heading that appears before this block
        heading = "Unknown"
...
def heading_to_filename(index: int, heading: str) -> str:
    """Convert a heading like 'Teil 1 — Aufgabe 41' into '01_Teil1_Aufgabe41'."""
    h = heading.replace("—", "").replace("&", "and")
    # Collapse whitespace around Teil/Aufgabe/QA tokens
    h = re.sub(r"Teil\s+", "Teil", h)
    h = re.sub(r"Aufgabe\s+", "Aufgabe", h)
    h = re.sub(r"Q\s*&?\s*A\s*", "QA", h)
    # Remove remaining whitespace
    h = re.sub(r"\s+", "_", h.strip())
    # Remove double underscores
    h = re.sub(r"_+", "_", h).strip("_")
    return f"{index + 1:02d}_{h}"


# ---------------------------------------------------------------------------
# Sentence extraction for SRT
# ---------------------------------------------------------------------------
...
def generate_srt(sentences: list[dict], block_start: float) -> str:
    ...
    for i, sent in enumerate(sentences, 1):
        rel_start = sent["start"] - block_start
        rel_end = sent["end"] - block_start
        # Clamp to zero
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python split_and_subtitle.py <path-to-generated-file.md>")
        sys.exit(1)
    ...
    repo_root = md_path.parent.parent  # Outputs/../ = repo root
    ...
    print(f"\n{'=' * 60}")
    print(f"  Audio Splitter & Subtitle Generator")
    ...
    for i, block in enumerate(blocks):
        ...
        if sentences:
            ...
            print(f"         ❌ SRT generation FAILED (no sentences found)")
